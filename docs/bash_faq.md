...
!!! note "Основы написания скриптов"

    | Элемент | Синтаксис | Пример |
    | --- | --- | --- |
    | Shebang | `#!/bin/bash` | Указывает интерпретатор |
    | Переменные | `VAR=value` | `NAME="test"` |
    | Обращение | `$VAR` или `${VAR}` | `echo $NAME` |
    | Код возврата | `$?` | `0` = успех, `≠0` = ошибка |
    | Условия | `if [ ]; then ... fi` | `if [ $x -eq 0 ]` |
    | Циклы | `for ... do ... done` | `for i in 1 2 3` |
    | Функции | `func() { }` | `hello() { echo Hi; }` |
