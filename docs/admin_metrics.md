...
!!! note "Метрики четырех ключевых показателей (DORA)"
    ...
    **Deployment Frequency** — Частота развертываний, частота попадания изменений в PROD.

    **Mean Time to Restore (MTTR)** — Среднее время восстановления, скорость исправления инцидентов.
